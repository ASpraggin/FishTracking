...
However, we quickly run into the issue that the program doesn't respect continuity. When the program can't figure out which fish is which after an overlap, Figure~\ref{fig:continuity}, it takes a guess based off distances, Figure~\ref{fig:broverlap}, but can get them wrong such as the situation on the right.
...
If the software mistakes the positions of the fish, which we will refer to as a swap, it will look like the fish traveled longer than it should have. Below we have the fish as it tracks up to an overlap and goes past it, so you can see where the error would occur.
