...
%replace first sentence with: Fish schooling is visually pleasing, schooling is challenging. in order to make decisions ,need quantitive data/where each fish is. it is easy to have data on fish, have person watch. if have to have person watch fish, can't get enough data for things like doing analysis. now talk about how advances in computer vision make haveing a program that allows for tracking of fish is game changer. 
Something that has perplexed animal scientists for some time now has been tracking schooling behavior in a meaningful way. \iffalse make this sentence more clear:\fi However, there has been some inroads made recently on the subject, primarily due to increases in technology. These advances will allow scientists to finally be able to do predictive modeling of the fish, as doing any sort of analysis before required manual observation.
